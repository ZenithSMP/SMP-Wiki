---
icon: swords
layout:
  width: default
  title:
    visible: true
  description:
    visible: true
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: true
  metadata:
    visible: true
---

# Skills

{% hint style="info" %}
This page was generated with the help of AI. Therefore, it may not be completely accurate.
{% endhint %}

Skills are leveled by performing related activities (mining, combat, farming, etc).\
Each skill grants **stats** and/or **effects** as you level up.

### Commands

* `/skills` - Opens the main skills menu
* `/skills stats` - Shows your current skill levels and bonuses.
* `/skills top` - Displays the top players for each skill.

***

<details>

<summary><strong>Combat</strong></summary>

**How to gain XP:** Deal damage to mobs and players\
**Max level:** 50

#### Stats

* **Strength** – Increases melee damage
* **Crit Chance** – Chance to land a critical hit
* **Crit Damage** – Increased damage on critical hits
* **Attack Speed** – Faster attacks

#### Effects

* **Serrated Strikes** – Chance to apply Bleed on hit
* **Strong Impact** – Chance to knock enemies back

</details>

<details>

<summary><strong>Armory</strong></summary>

**How to gain XP:** Take damage\
**Max level:** 50

#### Stats

* **Health** – Increased maximum health
* **Defense** – Reduced incoming damage
* **Speed** – Increased movement speed

#### Effects

* **Mystic Resilience** – Chance to gain Resistance when hit
* **Second Chance** – Chance to avoid fatal damage (cooldown applies)

</details>

<details>

<summary><strong>Mining</strong></summary>

**How to gain XP:** Break natural blocks\
**Max level:** 50

#### Stats

* **Defense** – Reduced damage taken
* **Ferocity** – Chance to deal damage twice

#### Effects

* **Versatile Tools** – Increased pickaxe damage
* **Spelunking** – Chance for bonus ore drops
* **Dynamic Mining** – Chance to gain Haste while mining

</details>

<details>

<summary><strong>Woodcutting</strong></summary>

**How to gain XP:** Break logs\
**Max level:** 50

#### Stats

* **Health** – Increased max health
* **Strength** – Increased melee damage

#### Effects

* **Master Lumberjack** – Chance for double log drops
* **Craftsmanship** – Chance to not consume axe durability

</details>

<details>

<summary><strong>Farming</strong></summary>

**How to gain XP:** Harvest crops\
**Max level:** 50

#### Stats

* **Speed** – Increased movement speed
* **Wisdom** – Increased skill XP gain

#### Effects

* **Bountiful Harvest** – Chance for extra crop drops
* **Satiation** – Chance to restore hunger
* **Golden Yield** – Chance for bonus drops

</details>

<details>

<summary><strong>Fishing</strong></summary>

**How to gain XP:** Fish\
**Max level:** 50

#### Stats

* **Wisdom** – Increased skill XP gain
* **Ferocity** – Chance to attack twice

#### Effects

* **Magnetic Rod** – Chance for extra fishing loot

</details>

<details>

<summary><strong>Alchemy</strong></summary>

**How to gain XP:** Brew potions\
**Max level:** 50

#### Stats

* **Wisdom** – Increased skill XP gain

#### Effects

* **Efficient Brewing** – Chance to not consume ingredients
* **Potent Mixtures** – Increased potion duration

</details>

<details>

<summary>Enchanting</summary>

**How to gain XP:** Enchant items\
**Max level:** 50

#### Stats

* **Wisdom** – Increased skill XP gain

#### Effects

* **Arcane Insight** – Reduced enchanting costs
* **Empowered Enchants** – Slightly stronger enchantments

</details>

<details>

<summary><strong>Exploration</strong></summary>

**How to gain XP:** Travel and discover the world\
**Max level:** 50

#### Stats

* **Speed** – Increased movement speed
* **Health** – Increased max health

#### Effects

* **Adventurer's Luck** – Chance to find bonus loot
* **Endurance** – Reduced travel penalties

</details>
